  ...
  <view class="receipt-list">
    ...
    >
      <view v-if="list.length > 0">
        <view class="receipt-card" v-for="item in list" :key="item.id">
          <view class="card-header">
            <text class="receipt-no">{{ item.receipt_no }}</text>
          ...
          <view class="card-body">
            ...
            <view class="body-item">
              <text class="body-label">收款方式</text>
      ...
      <view v-else-if="!loading" class="empty-state">
        ...
        <text class="empty-text">暂无收款单</text>
      </view>
...
const statusMap = { pending: '待收款', confirmed: '已收款' }
...
const fetchList = async (isRefresh = false) => {
  ...
  loading.value = true
  try {
    const res = await financeApi.getReceipts({ page: isRefresh ? 1 : page.value, pageSize: 10 })
    if (res.code === 0) {
...
.receipt-list { height: 100vh; display: flex; flex-direction: column; background: #F5F7FA; }
...
.receipt-card {
  ...
  .card-header { display: flex; justify-content: space-between; align-items: center; padding: 20rpx 24rpx; border-bottom: 1rpx solid #F2F6FC; .receipt-no { font-size: 28rpx; font-weight: 600; color: #303133; } }
