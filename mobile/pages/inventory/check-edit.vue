        ...
        <view class="section">
          ...
          <view class="form-group">
            ...
            <view class="form-item">
              ...
              <input :value="form.checker_name" class="form-input" disabled />
            </view>
            <view class="form-item">
              <text class="form-label required">盘点时间</text>
          ...
          <view v-if="form.items.length > 0" class="items-wrap">
            <view v-for="(item, index) in form.items" :key="item.product_id" class="item-row-card">
              ...
              <view class="form-group row-group">
                ...
                <view class="form-item">
                  <text class="form-label">账面库存</text>
                  <text class="product-info-text">{{ formatQty(item.book_quantity) }}</text>
                </view>
                <view class="form-item">
                  <text class="form-label required">实际库存</text>
                  <input v-model="item.actual_quantity" class="form-input" type="number" />
                </view>
                <view class="form-item">
                  <text class="form-label">差异</text>
                  <text class="product-info-text" :class="deltaClass(item.actual_quantity - item.book_quantity)">
                    {{ formatDelta(item.actual_quantity - item.book_quantity) }}
                  </text>
                </view>
                <view class="form-item">
                  <text class="form-label">备注</text>
                  <input v-model="item.remark" class="form-input" placeholder="行备注" />
                </view>
              </view>
            </view>
          </view>
...
function onDateChange(e) {
  ...
  form.check_time = `${e.detail.value} 00:00:00`
}
