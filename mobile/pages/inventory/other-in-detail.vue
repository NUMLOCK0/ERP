      ...
      <view class="scroll-inner" v-if="detail.id">
        ...
        <view class="summary-section">
          <view class="summary-header">
            <text class="inbound-no font-bold">{{ detail.inbound_no }}</text>
          ...
          <view class="summary-grid">
            <view class="grid-item">
              <text class="grid-label">供应商</text>
              <text class="grid-value font-bold">{{ detail.supplier_name || '-' }}</text>
            </view>
            <view class="grid-item">
            ...
            <view class="grid-item">
              <text class="grid-label">入库总金额</text>
            ...
            <view class="grid-item wide" v-if="detail.inbound_remark || detail.remark">
              <text class="grid-label">入库备注</text>
              <text class="grid-value">{{ detail.inbound_remark || detail.remark }}</text>
            </view>
          </view>
        </view>
        ...
        <view class="section-title-row">
          <text class="section-title">入库明细 ({{ detail.items?.length || 0 }} 笔)</text>
        </view>
        ...
        <view class="items-list">
          <view class="item-card" v-for="(item, idx) in detail.items" :key="idx">
            ...
            <view class="item-details">
              ...
              <view class="detail-row">
                <text class="row-label">入库单价：</text>
                <text class="row-value">￥{{ formatPrice(item.price) }}</text>
              </view>
              <view class="detail-row">
                <text class="row-label">入库总价：</text>
...
onMounted(async () => {
  if (detailId.value) {
    try {
      const res = await inventoryApi.getOtherInDetail(detailId.value)
      if (res.code === 0) {
