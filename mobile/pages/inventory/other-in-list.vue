  ...
  <view class="list-page">
    <view class="search-bar">
      <view class="search-input-wrap">
        ...
        <input v-model="keyword" class="search-input" placeholder="搜索其他入库单号/备注" type="text" confirm-type="search" @confirm="reload" />
      </view>
      ...
      <view class="scroll-inner">
        <view v-if="list.length > 0" class="card-list">
          <BusinessListItem
            ...
            :title="item.other_in_no || item.inbound_no || item.in_no || '-'"
          ...
          >
            ...
            <template #detail>
              <view class="detail-section">
                <text class="detail-title">其他入库信息</text>
                <view class="detail-grid">
                  <view class="detail-row"><text class="detail-label">单号</text><text class="detail-value">{{ item.inbound_no || '-' }}</text></view>
        ...
        <view v-else-if="!loading" class="empty-state"><u-icon name="grid" size="60" color="#DCDFE6"  /><text class="empty-text">暂无其他入库单</text></view>
...
async function fetchList(reset = false) { if (loading.value) return; loading.value = true; try { const res = await inventoryApi.getOtherInList({ page: reset ? 1 : page.value, pageSize, keyword: keyword.value.trim() }); const data = res.data?.list || res.data || []; const total = res.data?.total ?? data.length; list.value = reset ? data : [...list.value, ...data]; page.value = reset ? 2 : page.value + 1; noMore.value = list.value.length >= total } finally { loading.value = false; refreshing.value = false } }
...
function goCreate() { uni.navigateTo({ url: '/pages/inventory/other-in-edit' }) }
function goDetail(id) { uni.navigateTo({ url: `/pages/inventory/other-in-detail?id=${id}` }) }
