  ...
  <view class="transfer-list">
    ...
    >
      <view v-if="list.length > 0">
        <view class="transfer-card" v-for="item in list" :key="item.id">
          ...
          <view class="card-body">
            <view class="body-item">
              ...
              <text class="body-value">{{ item.from_warehouse_name || item.fromWarehouseName || '-' }}</text>
            </view>
            <view class="body-item">
              <text class="body-label">调入仓库</text>
              <text class="body-value">{{ item.to_warehouse_name || item.toWarehouseName || '-' }}</text>
            </view>
            <view class="body-item">
      ...
      <view v-else-if="!loading" class="empty-state">
        <uni-icons type="redo" size="60" color="#DCDFE6"></uni-icons>
...
const fetchList = async (isRefresh = false) => {
  ...
  loading.value = true
  try {
    const res = await inventoryApi.getTransfers({ page: isRefresh ? 1 : page.value, pageSize: 10 })
    if (res.code === 0) {
...
.transfer-list { height: 100vh; display: flex; flex-direction: column; background: #F5F7FA; }
...
.transfer-card {
  ...
  .card-header { display: flex; justify-content: space-between; align-items: center; padding: 20rpx 24rpx; border-bottom: 1rpx solid #F2F6FC; .transfer-no { font-size: 28rpx; font-weight: 600; color: #303133; } }
