<template>
  <view class="brand-page" @touchmove.stop.prevent="() => {}">
    <!-- 搜索栏 -->
        ...
        <view class="dialog-body">
          <view class="form-group">
            <view class="form-item">
              <text class="form-label required">品牌名称</text>
              <input class="form-input" v-model="form.name" placeholder="请输入品牌名称" />
            </view>
            <view class="form-item">
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await brandApi.getList({ keyword: keyword.value.trim() })
    if (res.code === 0) {
...
const handleDelete = (item) => {
  uni.showModal({
    ...
    success: async (res) => {
      if (res.confirm) {
        try {
          const deleteRes = await brandApi.delete(item.id)
          if (deleteRes.code === 0) {
...
const handleSubmit = async () => {
  ...
  try {
    let res
    if (editId.value) {
      res = await brandApi.update(editId.value, payload)
    } else {
      res = await brandApi.create(payload)
    }
