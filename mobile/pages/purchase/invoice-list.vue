  ...
  <view class="invoice-list">
    ...
    <view class="filter-bar">
      <view class="filter-item">
        ...
        <picker class="filter-picker" @change="onSupplierFilterChange" :value="supplierIndex" :range="suppliers" range-key="name">
          ...
          >
            <view class="card-header">
              ...
              <view class="header-tags">
                ...
                <uni-tag :text="getPaymentStatusText(item)" size="small" :type="getPaymentStatusType(item)" />
              </view>
            </view>
            <view class="card-body">
              ...
              <view class="body-item">
                <text class="body-label">关联采购单</text>
              ...
              <view class="body-item">
                <text class="body-label">已付金额</text>
                <text class="body-value text-success font-bold">¥{{ formatPrice(item.paid_amount) }}</text>
              </view>
              <view class="body-item">
...
const getPaymentStatusText = (item) => {
  ...
  const paidVal = Number(item.paid_amount || 0)
  if (paidVal <= 0) return '未付款'
  if (paidVal >= totalVal) return '已付清'
  return '部分付款'
}

const getPaymentStatusType = (item) => {
  const totalVal = Number(item.total_amount || 0)
  const paidVal = Number(item.paid_amount || 0)
...
const fetchList = async (isRefresh = false) => {
  ...
  try {
    ...
    const res = await purchaseApi.getInvoices(params)
    if (res.code === 0) {
...
const loadSuppliers = async () => {
  try {
    const res = await commonApi.getSuppliers({ page: 1, pageSize: 1000, type: 'supplier', status: 1 })
    if (res.code === 0) {
...
const onSupplierFilterChange = (e) => {
  ...
  selectedSupplierId.value = suppliers.value[index]?.id || null
  onSearch()
}
...
const goDetail = (id) => {
  uni.navigateTo({ url: `/pages/purchase/invoice-detail?id=${id}` })
}

const goCreate = () => {
  uni.navigateTo({ url: `/pages/purchase/invoice-edit` })
}
