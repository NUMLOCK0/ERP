        ...
        <view class="return-card" v-for="item in list" :key="item.id">
          ...
          <view class="card-body">
            <view class="body-item">
              <text class="body-label">关联入库单</text>
              <text class="body-value">{{ item.inbound_no || '-' }}</text>
            </view>
            <view class="body-item">
...
const fetchList = async (isRefresh = false) => {
  ...
  loading.value = true
  try {
    const res = await purchaseApi.getReturns({ page: isRefresh ? 1 : page.value, pageSize: 10 })
    if (res.code === 0) {
