<template>
  <view class="report-page">
            ...
            <template #detail>
              <view class="detail-section">
                ...
                <view class="detail-grid">
                  ...
                  <view class="detail-row"><text class="detail-label">采购数量</text><text class="detail-value">{{ formatQuantity(item.purchase_quantity || item.quantity) }}</text></view>
    ...
    <ReportSummaryBar title="仓库采购汇总" :items="summaryItems" />
  </view>
</template>
...
const statusMap = { 0: '草稿', 1: '采购中', 2: '待审核', 3: '已取消', 4: '已关闭', 5: '已审核', 8: '已完成', draft: '草稿', submitted: '待审核', approved: '已审核', completed: '已完成', cancelled: '已取消' }
...
const getStatusType = (status) => ({ 0: 'info', 1: 'primary', 2: 'warning', 3: 'error', 4: 'default', 5: 'success', 8: 'success', draft: 'info', submitted: 'warning', approved: 'success', completed: 'success', cancelled: 'error' }[status] || 'info')
...
const mapSummaryItems = (rows) => rows.map((item, index) => ({
  ...
  stats: [
    ...
    { label: '最终入库总额', value: `￥${formatPrice(item.final_inbound_amount)}` },
...
const fetchSummary = async () => {
  try {
    const res = await reportApi.getPurchaseOrderSummary()
    if (res.code === 0) {
...
const fetchList = async (reset = false) => {
  ...
  try {
    ...
    const res = await reportApi.getPurchaseOrder({ page: currentPage, pageSize: 20 })
    if (res.code === 0) {
      const data = res.data?.list || res.data || []
