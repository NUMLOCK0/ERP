<template>
  <view class="report-page">
    <view class="header-bar">
      <text class="report-title">采购订单报表</text>
    </view>
...
const getStatusType = (s) => {
  const m = { draft: 'info', submitted: 'warning', approved: 'success', completed: '', cancelled: 'error' }
  return m[s] || 'info'
}
...
const fetchList = async () => {
  ...
  loading.value = true
  try {
    const res = await reportApi.getPurchaseOrder({ page: page.value, pageSize: 20 })
    if (res.code === 0) {
