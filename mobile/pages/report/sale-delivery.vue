<template>
  <view class="report-page">
      ...
      <view class="scroll-inner with-summary">
        <view v-if="list.length" class="card-list">
          <BusinessListItem
            ...
            :tag-text="deliveryStatusText(item.status)"
            :tag-type="deliveryStatusType(item.status)"
          ...
          >
            <template #summary>
              <view class="summary-grid">
                ...
                <view class="summary-item"><text class="summary-label">发货总额</text><text class="summary-value amount">￥{{ formatPrice(item.sale_total_amount || item.total_amount) }}</text></view>
              </view>
            </template>
            <template #detail>
              <view class="detail-section">
                <text class="detail-title">发货报表明细</text>
                <view class="detail-grid">
                  ...
                  <view class="detail-row"><text class="detail-label">发货数量</text><text class="detail-value">{{ formatQuantity(item.delivery_quantity || item.quantity) }}</text></view>
    ...
    <ReportSummaryBar title="仓库发货汇总" :items="summaryItems" />
  </view>
</template>
...
const deliveryStatusText = (status) => ({ 0: '待发货', 1: '已发货', 2: '已签收', 3: '已取消', confirmed: '已确认', pending: '待确认' }[status] || status || '未知')
const deliveryStatusType = (status) => ({ 0: 'warning', 1: 'primary', 2: 'success', 3: 'error', confirmed: 'success', pending: 'warning' }[status] || 'info')
...
const mapSummaryItems = (rows) => rows.map((item, index) => ({
  ...
  stats: [
    ...
    { label: '发货总数', value: formatQuantity(item.delivery_total) },
    { label: '发货总额', value: `￥${formatPrice(item.total_amount)}` },
    { label: '税金总额', value: `￥${formatPrice(item.total_tax)}` },
...
const fetchSummary = async () => {
  try {
    const res = await reportApi.getSaleDeliverySummary()
    if (res.code === 0) {
...
const fetchList = async (reset = false) => {
  ...
  try {
    ...
    const res = await reportApi.getSaleDelivery({ page: currentPage, pageSize: 20 })
    if (res.code === 0) {
      const data = res.data?.list || res.data || []
