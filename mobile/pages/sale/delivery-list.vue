  ...
  <view class="delivery-list">
    ...
    >
      <view v-if="list.length > 0">
        <view class="delivery-card" v-for="item in list" :key="item.id">
          <view class="card-header">
            <text class="delivery-no">{{ item.delivery_no }}</text>
          ...
          <view class="card-body">
            ...
            <view class="body-item" v-if="item.logistics_company">
              <text class="body-label">物流</text>
              <text class="body-value">{{ item.logistics_company }} {{ item.logistics_no || '' }}</text>
            </view>
          </view>
      ...
      <view v-else-if="!loading" class="empty-state">
        <uni-icons type="paperplane" size="60" color="#DCDFE6"></uni-icons>
        <text class="empty-text">暂无发货单</text>
      </view>
...
const fetchList = async (isRefresh = false) => {
  ...
  loading.value = true
  try {
    const res = await saleApi.getDeliveries({ page: isRefresh ? 1 : page.value, pageSize: 10 })
    if (res.code === 0) {
...
.delivery-list { height: 100vh; display: flex; flex-direction: column; background: #F5F7FA; }
...
.delivery-card {
  ...
  .card-header { display: flex; justify-content: space-between; align-items: center; padding: 20rpx 24rpx; border-bottom: 1rpx solid #F2F6FC; .delivery-no { font-size: 28rpx; font-weight: 600; color: #303133; } }
