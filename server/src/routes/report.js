...
router.get('/purchase-inbound', (req, res) => {
  try {
    const db = getDatabase();
    ...
    const total = db.prepare(`SELECT COUNT(*) AS cnt FROM purchase_inbound pi LEFT JOIN supplier_customer sc ON pi.supplier_id = sc.id LEFT JOIN warehouse w ON pi.warehouse_id = w.id WHERE ${where}`).get(...params).cnt;
    const list = db.prepare(`SELECT pi.*, sc.name AS supplier_name, w.name AS warehouse_name FROM purchase_inbound pi LEFT JOIN supplier_customer sc ON pi.supplier_id = sc.id LEFT JOIN warehouse w ON pi.warehouse_id = w.id WHERE ${where} ORDER BY pi.id DESC LIMIT ? OFFSET ?`).all(...params, Number(pageSize), (page - 1) * pageSize);
    res.json(Response.paginate(list, total, page, pageSize));
  } catch (err) {
    res.json(Response.error(err.message));
  }
...
router.get('/sale-delivery', (req, res) => {
  try {
    const db = getDatabase();
    const { page = 1, pageSize = 20, start_date = '', end_date = '', warehouse_id = '' } = req.query;
    let where = '1=1';
    ...
    const total = db.prepare(`SELECT COUNT(*) AS cnt FROM sale_delivery sd LEFT JOIN supplier_customer sc ON sd.customer_id = sc.id LEFT JOIN warehouse w ON sd.warehouse_id = w.id WHERE ${where}`).get(...params).cnt;
    const list = db.prepare(`SELECT sd.*, sc.name AS customer_name, w.name AS warehouse_name FROM sale_delivery sd LEFT JOIN supplier_customer sc ON sd.customer_id = sc.id LEFT JOIN warehouse w ON sd.warehouse_id = w.id WHERE ${where} ORDER BY sd.id DESC LIMIT ? OFFSET ?`).all(...params, Number(pageSize), (page - 1) * pageSize);
    res.json(Response.paginate(list, total, page, pageSize));
  } catch (err) {
    res.json(Response.error(err.message));
  }
...
router.get('/purchase-order', (req, res) => {
  try {
    ...
    const list = db.prepare(`SELECT po.*, sc.name AS supplier_name FROM purchase_order po LEFT JOIN supplier_customer sc ON po.supplier_id = sc.id WHERE ${where} ORDER BY po.id DESC LIMIT ? OFFSET ?`).all(...params, Number(pageSize), (page - 1) * pageSize);
    res.json(Response.paginate(list, total, page, pageSize));
  } catch (err) {
    res.json(Response.error(err.message));
  }
...
router.get('/sale-order', (req, res) => {
  try {
    ...
    const list = db.prepare(`SELECT so.*, sc.name AS customer_name FROM sale_order so LEFT JOIN supplier_customer sc ON so.customer_id = sc.id WHERE ${where} ORDER BY so.id DESC LIMIT ? OFFSET ?`).all(...params, Number(pageSize), (page - 1) * pageSize);
    res.json(Response.paginate(list, total, page, pageSize));
  } catch (err) {
    res.json(Response.error(err.message));
  }
...
router.get('/payment', (req, res) => {
  try {
    ...
    const list = db.prepare(`SELECT fp.*, sc.name AS supplier_name FROM finance_payment fp LEFT JOIN supplier_customer sc ON fp.supplier_id = sc.id WHERE ${where} ORDER BY fp.id DESC LIMIT ? OFFSET ?`).all(...params, Number(pageSize), (page - 1) * pageSize);
    res.json(Response.paginate(list, total, page, pageSize));
  } catch (err) {
    res.json(Response.error(err.message));
  }
...
router.get('/receipt', (req, res) => {
  try {
    ...
    const list = db.prepare(`SELECT fr.*, sc.name AS customer_name FROM finance_receipt fr LEFT JOIN supplier_customer sc ON fr.customer_id = sc.id WHERE ${where} ORDER BY fr.id DESC LIMIT ? OFFSET ?`).all(...params, Number(pageSize), (page - 1) * pageSize);
    res.json(Response.paginate(list, total, page, pageSize));
  } catch (err) {
    res.json(Response.error(err.message));
  }
...
router.get('/inbound-summary', (req, res) => {
  try {
    const db = getDatabase();
    ...
    const list = db.prepare(`
      ...
      WHERE ${where}
      GROUP BY pii.product_id ORDER BY total_amount DESC
    `).all(...params);
...
router.get('/outbound-summary', (req, res) => {
  try {
    const db = getDatabase();
    const { start_date = '', end_date = '' } = req.query;
    let where = '1=1';
    ...
    const list = db.prepare(`
      ...
      WHERE ${where}
      GROUP BY sdi.product_id ORDER BY total_amount DESC
    `).all(...params);
    res.json(Response.success(list));
  } catch (err) {
    res.json(Response.error(err.message));
