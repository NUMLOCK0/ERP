  ...
  <div class="page-container">
    <el-card>
      ...
      <el-table :data="tableData" stripe v-loading="loading">
        ...
        <el-table-column prop="created_at" label="入职时间" width="120" />
...
const form = reactive({ name: '', phone: '', position: '', department: '', status: 1 })
const rules = { name: [{ required: true, message: '请输入姓名', trigger: 'blur' }] }

async function fetchData() {
  loading.value = true
  try { const res: any = await getEmployees(); tableData.value = res.data || [] } finally { loading.value = false }
}
...
function handleEdit(row: any) {
  ...
  Object.assign(form, { name: row.name, phone: row.phone||'', position: row.position||'', department: row.department||'', status: row.status })
...
function resetForm() { Object.assign(form, { name: '', phone: '', position: '', department: '', status: 1 }); formRef.value?.clearValidate() }
