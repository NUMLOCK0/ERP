  ...
  <div class="page-container">
    <template v-if="editorVisible">
      <div class="editor-page">
        ...
        <el-form ref="formRef" :model="form" :rules="formRules" label-width="96px" class="inbound-form">
          <div class="inbound-form-grid">
            <el-form-item label="入库单号" required>
              <el-input v-model="form.inbound_no" disabled />
            </el-form-item>
            <el-form-item label="供应商" prop="supplier_id" required>
              <el-select v-model="form.supplier_id" placeholder="请选择..." clearable filterable>
            ...
            <el-form-item label="单据备注">
              <el-input v-model="form.inbound_remark" type="textarea" :rows="1" placeholder="入库备注" />
            </el-form-item>
          </div>
          ...
          <el-table border :data="form.items" class="inbound-detail-table" stripe>
            <template #empty>
              <div class="empty-inbound">
            ...
            <el-table-column label="入库数量" width="140" align="right">
              <template #header>
                <span>入库数量 <span class="edit-mark">✎</span></span>
              </template>
              <template #default="{ row }">
                <el-input-number v-model="row.quantity" :min="0.01" :precision="2" :controls="false" @change="recalculateRow(row)" />
              </template>
            </el-table-column>
            <el-table-column label="入库价格" width="140" align="right">
              <template #header>
                <span>入库价格 <span class="edit-mark">✎</span></span>
              </template>
              <template #default="{ row }">
                <el-input-number v-model="row.price" :min="0" :precision="2" :controls="false" @change="recalculateRow(row)" />
              </template>
            </el-table-column>
            <el-table-column label="入库总价" width="130" align="right">
        ...
        <div class="editor-footer">
          <div class="total-bar">
            ...
            <span>产品总数量：{{ formatQuantity(totalQuantity) }}</span>
      ...
      <el-drawer v-model="productDrawerVisible" title="选择产品" size="62%" direction="rtl">
        ...
        <el-table border :data="drawerProducts" class="drawer-product-table" height="calc(100vh - 190px)" stripe>
          ...
          <el-table-column label="入库价" width="120" align="right">
            <template #default="{ row }">¥{{ formatMoney(productPrice(row)) }}</template>
          </el-table-column>
        </el-table>
    ...
    <el-card v-else>
      ...
      <TableColumnTools table-key="inventory-other-inbound" filename="其他入库单" />

      <el-table border :data="tableData" stripe v-loading="loading">
        <el-table-column prop="id" label="入库单id" width="100" />
        <el-table-column prop="inbound_no" label="入库单号" width="190" show-overflow-tooltip>
        ...
        <el-table-column prop="inbound_remark" label="入库备注信息" width="180" show-overflow-tooltip>
          <template #default="{ row }">{{ emptyText(row.inbound_remark || row.remark) }}</template>
        </el-table-column>
    ...
    <el-drawer v-model="detailDrawerVisible" title="其他入库单详情" size="72%" direction="rtl" class="detail-drawer">
      <el-tabs v-model="detailActiveTab">
        <el-tab-pane label="基础信息" name="basic">
          <el-descriptions :column="2" border>
            <el-descriptions-item label="入库单ID">{{ emptyText(detailData.id) }}</el-descriptions-item>
            <el-descriptions-item label="入库单号"><CopyableNo :value="detailData.inbound_no" /></el-descriptions-item>
            ...
            <el-descriptions-item label="入库总金额">¥{{ formatMoney(detailData.total_amount) }}</el-descriptions-item>
            <el-descriptions-item label="管理员备注信息">{{ emptyText(detailData.admin_remark) }}</el-descriptions-item>
            <el-descriptions-item label="入库备注信息">{{ emptyText(detailData.inbound_remark || detailData.remark) }}</el-descriptions-item>
        ...
        <el-tab-pane label="产品列表" name="items">
          <el-table border :data="detailData.items" stripe class="detail-item-table">
            ...
            <el-table-column label="入库总价" width="120" align="right">
              <template #default="{ row }">¥{{ formatMoney(row.amount) }}</template>
            </el-table-column>
          </el-table>
        </el-tab-pane>
...
import { createOtherInbound, getOtherInbound, getOtherInbounds } from '@/api/inventory'
...
async function fetchData() {
  ...
  try {
    ...
    const res: any = await getOtherInbounds(params)
...
async function handleView(row: any) {
  const res: any = await getOtherInbound(row.id)
...
function createItem(): InboundItem {
  return {
    product_id: null,
    product_name: '',
    code: '',
...
function fillRowFromProduct(row: InboundItem, product: any, quantity = 1) {
...
function recalculateRow(row: InboundItem) {
...
async function handleSave() {
  ...
  try {
    await createOtherInbound({
      ...
      inbound_remark: form.inbound_remark,
...
function filterProducts(keyword = '', categoryId: any = '', brandId: any = '') {
  ...
  return products.value.filter(product => {
    const matchedText = !text || [product.name, product.code, product.barcode, product.spec, product.description]
...
.inbound-form-grid :deep(.el-select),
.inbound-form-grid :deep(.el-input) {
  width: 100%;
}
...
.inbound-detail-table :deep(.el-table__empty-block) {
  min-height: 320px;
}

.inbound-detail-table :deep(.el-table__empty-text) {
  width: 100%;
}
