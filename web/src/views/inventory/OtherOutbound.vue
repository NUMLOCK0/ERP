<template>
  <div class="page-container">
    <el-card>
      <div class="toolbar">
        <el-button type="primary" :icon="Plus" @click="ElMessage.info('新增其他出库单')">新增其他出库单</el-button>
      </div>
      <SearchForm :model="searchForm" @search="handleSearch" @reset="handleReset">
        <el-form-item label="出库单号"><el-input v-model="searchForm.outbound_no" placeholder="出库单号" clearable /></el-form-item>
...
import { getOtherOutbounds } from '@/api/inventory'
...
const searchForm = reactive({ outbound_no: '', warehouse_id: null as any })

async function fetchData() {
  loading.value = true
  try {
    const res: any = await getOtherOutbounds({ page: pagination.page, pageSize: pagination.size, ...searchForm })
    tableData.value = res.data?.list || []; total.value = res.data?.total || 0
  } finally { loading.value = false }
}
function handleSearch() { pagination.page = 1; fetchData() }
function handleReset() { Object.assign(searchForm, { outbound_no: '', warehouse_id: null }); handleSearch() }
