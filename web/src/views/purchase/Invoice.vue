<template>
  <div class="page-container">
    <el-card>
      <SearchForm :model="searchForm" @search="handleSearch" @reset="handleReset">
        <el-form-item label="关键词">
          <el-input v-model="searchForm.keyword" placeholder="登记单号/发票号/采购单号/供应商" clearable />
        </el-form-item>
      ...
      <div class="toolbar">
        ...
        <div class="toolbar-right">
          <TableColumnTools table-key="purchase-invoice" filename="采购发票登记" />
        </div>
      </div>
    ...
    <el-drawer v-model="createDrawerVisible" title="采购发票登记" size="560px" direction="rtl" @close="resetForm">
      <el-form ref="formRef" :model="form" :rules="formRules" label-width="110px" class="invoice-form">
        <el-form-item label="采购订单" prop="order_id">
          <el-select v-model="form.order_id" placeholder="请选择采购订单" filterable clearable @change="handleOrderChange">
            <el-option v-for="order in orders" :key="order.id" :label="`${order.order_no} / ${order.supplier_name || ''}`" :value="order.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="供应商">
    ...
    <el-drawer v-model="detailDrawerVisible" title="采购发票详情" size="560px" direction="rtl">
      <el-descriptions :column="1" border>
        ...
        <el-descriptions-item label="采购单号">{{ detailData.order_no }}</el-descriptions-item>
...
import { createPurchaseInvoice, getPurchaseInvoice, getPurchaseInvoices, getPurchaseOrders } from '@/api/purchase'
...
const searchForm = reactive({ keyword: '', supplier_id: null as any })
const form = reactive({
...
async function fetchData() {
  loading.value = true
  try {
    const res: any = await getPurchaseInvoices({ page: pagination.page, pageSize: pagination.size, ...searchForm })
...
function handleReset() { Object.assign(searchForm, { keyword: '', supplier_id: null }); handleSearch() }
...
async function openDetailDrawer(row: any) {
  const res: any = await getPurchaseInvoice(row.id)
...
onMounted(async () => {
  fetchData()
  const [supplierRes, orderRes]: any[] = await Promise.all([
    getSuppliers({ page: 1, pageSize: 1000, type: 'supplier', status: 1 }),
    getPurchaseOrders({ page: 1, pageSize: 1000 })
  ])
