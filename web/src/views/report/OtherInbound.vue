<template>
  <div class="report-page">
    <el-card>
      ...
      <TableColumnTools table-key="report-other-inbound" filename="其他入库报表" />

      <el-table border :data="tableData" stripe v-loading="loading">
        <el-table-column prop="id" label="数据ID" width="90" />
        <el-table-column prop="inbound_no" label="其他入库单号" width="190" show-overflow-tooltip>
        ...
        <el-table-column label="入库总额" width="120" align="right">
          <template #default="{ row }">¥{{ formatMoney(row.inbound_total_amount) }}</template>
        </el-table-column>
    ...
    <el-drawer v-model="detailVisible" title="其他入库详情" size="72%">
      <div v-loading="detailLoading" class="detail-content">
        <template v-if="detail">
          <el-descriptions :column="3" border>
            <el-descriptions-item label="数据ID">{{ detail.report_item_id }}</el-descriptions-item>
            <el-descriptions-item label="其他入库单号"><CopyableNo :value="detail.inbound_no" /></el-descriptions-item>
            ...
            <el-descriptions-item label="入库总数">{{ formatQuantity(detail.total_quantity) }}</el-descriptions-item>
            <el-descriptions-item label="入库总额">¥{{ formatMoney(detail.total_amount) }}</el-descriptions-item>
            ...
            <el-descriptions-item label="入库备注" :span="3">{{ emptyText(detail.inbound_remark || detail.remark) }}</el-descriptions-item>
          </el-descriptions>
    ...
    <el-dialog v-model="warehouseDialogVisible" title="全部仓库其他入库数据" width="850px">
...
import { getOtherInboundReport, getOtherInboundWarehouseSummary } from '@/api/report'
import { getOtherInbound } from '@/api/inventory'
...
async function fetchData() {
  loading.value = true
  try {
    const res: any = await getOtherInboundReport(reportParams())
...
async function fetchWarehouseSummaries() {
  const res: any = await getOtherInboundWarehouseSummary(reportParams(false))
...
async function handleDetail(row: any) {
  ...
  try {
    const res: any = await getOtherInbound(row.inbound_id)
    detail.value = {
      ...row,
      ...(res.data || {}),
