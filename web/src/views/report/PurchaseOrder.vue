<template>
  <div class="report-page">
    <el-card>
      <SearchForm :model="searchForm" @search="handleSearch" @reset="handleReset">
        <el-form-item label="关键词">
          <el-input v-model="searchForm.keyword" placeholder="采购单号/产品名称/编码" clearable />
        </el-form-item>
      ...
      <TableColumnTools table-key="report-purchase-order" filename="采购订单报表" />
      ...
      <el-table border :data="tableData" stripe v-loading="loading">
        ...
        <el-table-column label="入库价格" width="110" align="right">
          <template #default="{ row }">¥{{ formatMoney(row.inbound_price) }}</template>
        </el-table-column>
        ...
        <el-table-column label="最终采购价" width="120" align="right">
          <template #default="{ row }">¥{{ formatMoney(row.final_purchase_price) }}</template>
        </el-table-column>
        <el-table-column label="最终采购总额" width="130" align="right">
          <template #default="{ row }">¥{{ formatMoney(row.final_purchase_total_amount) }}</template>
        </el-table-column>
        <el-table-column label="最终采购数量" width="130" align="right">
          <template #default="{ row }">{{ formatQuantity(row.final_purchase_quantity) }}</template>
        </el-table-column>
        <el-table-column label="最终税金" width="110" align="right">
          <template #default="{ row }">¥{{ formatMoney(row.final_tax) }}</template>
        </el-table-column>
        <el-table-column label="最终税金总额" width="130" align="right">
          <template #default="{ row }">¥{{ formatMoney(row.final_tax_total) }}</template>
        </el-table-column>
        <el-table-column label="已正式入库数量" width="145" align="right">
          <template #default="{ row }">{{ formatQuantity(row.formal_inbound_quantity) }}</template>
        </el-table-column>
        ...
        <div class="warehouse-cards">
          <div v-for="warehouse in visibleWarehouses" :key="warehouse.id" class="warehouse-card">
            ...
            <div class="warehouse-stat-grid">
              ...
              <span>入库总额 ¥{{ formatMoney(warehouse.inbound_amount) }}</span>
              <span>税金总额 ¥{{ formatMoney(warehouse.tax_total) }}</span>
              <span>最终入库总额 ¥{{ formatMoney(warehouse.final_inbound_amount) }}</span>
              <span>最终税金总额 ¥{{ formatMoney(warehouse.final_tax_total) }}</span>
              <span>最终采购总数 {{ formatQuantity(warehouse.final_purchase_total) }}</span>
              <span>已正式入库总数 {{ formatQuantity(warehouse.formal_inbound_total) }}</span>
              <span>退款总额 ¥{{ formatMoney(warehouse.refund_total) }}</span>
              <span>退货总数 {{ formatQuantity(warehouse.return_total) }}</span>
            </div>
          </div>
    ...
    <el-drawer v-model="detailVisible" title="采购订单详情" size="78%">
      <div v-loading="detailLoading" class="detail-content">
        <template v-if="detail">
          <el-descriptions :column="3" border>
            ...
            <el-descriptions-item label="订单状态">{{ statusLabel(detail.status) }}</el-descriptions-item>
            ...
            <el-descriptions-item label="订单总额">¥{{ formatMoney(detail.total_amount) }}</el-descriptions-item>
            ...
            <el-descriptions-item label="管理员备注" :span="3">{{ emptyText(detail.admin_remark) }}</el-descriptions-item>
            <el-descriptions-item label="采购备注" :span="3">{{ emptyText(detail.purchase_remark) }}</el-descriptions-item>
          </el-descriptions>

          <div class="detail-section-title">采购产品</div>
          <el-table :data="detail.items || []" border stripe>
            ...
            <el-table-column prop="spec" label="产品规格" width="130" show-overflow-tooltip />
            ...
            <el-table-column label="最终采购总额" width="130" align="right">
              <template #default="{ row }">¥{{ formatMoney(finalAmount(row)) }}</template>
            </el-table-column>
            <el-table-column label="正式入库数量" width="130" align="right">
              <template #default="{ row }">{{ formatQuantity(row.formal_inbound_quantity) }}</template>
            </el-table-column>
            <el-table-column label="入库价格" width="110" align="right">
              <template #default="{ row }">¥{{ formatMoney(row.inbound_price) }}</template>
            ...
            <el-table-column label="退货数量" width="110" align="right">
              <template #default="{ row }">{{ formatQuantity(row.completed_return_quantity) }}</template>
            </el-table-column>
            <el-table-column label="退款金额" width="120" align="right">
              <template #default="{ row }">¥{{ formatMoney(row.refund_amount) }}</template>
            </el-table-column>
    ...
    <el-dialog v-model="warehouseDialogVisible" title="全部仓库采购入库数据" width="92%">
      <el-table border :data="warehouseSummaries" stripe max-height="520">
        ...
        <el-table-column label="入库总额" width="140" align="right">
          <template #default="{ row }">¥{{ formatMoney(row.inbound_amount) }}</template>
        </el-table-column>
        <el-table-column label="税金总额" width="140" align="right">
          <template #default="{ row }">¥{{ formatMoney(row.tax_total) }}</template>
        </el-table-column>
        <el-table-column label="最终入库总额" width="150" align="right">
          <template #default="{ row }">¥{{ formatMoney(row.final_inbound_amount) }}</template>
        </el-table-column>
        <el-table-column label="最终税金总额" width="150" align="right">
          <template #default="{ row }">¥{{ formatMoney(row.final_tax_total) }}</template>
        </el-table-column>
        <el-table-column label="最终采购总数" width="140" align="right">
          <template #default="{ row }">{{ formatQuantity(row.final_purchase_total) }}</template>
        </el-table-column>
        <el-table-column label="已正式入库总数" width="150" align="right">
          <template #default="{ row }">{{ formatQuantity(row.formal_inbound_total) }}</template>
        </el-table-column>
        <el-table-column label="退款总额" width="140" align="right">
          <template #default="{ row }">¥{{ formatMoney(row.refund_total) }}</template>
        </el-table-column>
        <el-table-column label="退货总数" width="120" align="right">
          <template #default="{ row }">{{ formatQuantity(row.return_total) }}</template>
        </el-table-column>
      </el-table>
    </el-dialog>
  </div>
...
import { getPurchaseOrderReport, getPurchaseOrderWarehouseSummary } from '@/api/report'
...
async function fetchData() {
  loading.value = true
  try {
    const res: any = await getPurchaseOrderReport(reportParams())
...
async function fetchWarehouseSummaries() {
  const res: any = await getPurchaseOrderWarehouseSummary(reportParams(false))
...
function firstProductImage(row: any) {
  return normalizeImageUrls(row.product_image_urls ?? row.image_urls)[0] || ''
}

function finalAmount(row: any) {
  if (row.final_amount !== undefined && row.final_amount !== null) return row.final_amount
  const quantity = Number(row.final_quantity ?? row.quantity ?? 0)
  const price = Number(row.final_price ?? row.price ?? 0)
  const tax = row.final_quantity !== undefined && row.final_quantity !== null
    ? Number(row.final_tax || 0)
    : Number(row.tax || 0)
  return quantity * price + tax
}

function statusLabel(status: any) {
  return statusOptions.find(option => option.value === Number(status))?.label || emptyText(status)
}
