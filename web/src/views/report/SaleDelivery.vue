<template>
  <div class="report-page">
    <el-card>
      <SearchForm :model="searchForm" @search="handleSearch" @reset="handleReset">
        <el-form-item label="关键词">
          <el-input v-model="searchForm.keyword" placeholder="发货单号/产品名称/编码" clearable />
        </el-form-item>
      ...
      <TableColumnTools table-key="report-sale-delivery" filename="销售发货报表" />
      ...
      <el-table border :data="tableData" stripe v-loading="loading">
        ...
        <el-table-column label="销售总额" width="120" align="right">
          <template #default="{ row }">¥{{ formatMoney(row.sale_total_amount) }}</template>
        </el-table-column>
        <el-table-column label="发货数量" width="110" align="right">
          <template #default="{ row }">{{ formatQuantity(row.delivery_quantity) }}</template>
        </el-table-column>
        ...
        <div class="warehouse-cards">
          <div v-for="warehouse in visibleWarehouses" :key="warehouse.id" class="warehouse-card">
            ...
            <div class="warehouse-stat-grid">
              ...
              <span>发货总数 {{ formatQuantity(warehouse.delivery_total) }}</span>
              <span>总额 ¥{{ formatMoney(warehouse.total_amount) }}</span>
              <span>总税金 ¥{{ formatMoney(warehouse.total_tax) }}</span>
    ...
    <el-drawer v-model="detailVisible" title="销售发货详情" size="72%">
      <div v-loading="detailLoading" class="detail-content">
        <template v-if="detail">
          <el-descriptions :column="3" border>
            ...
            <el-descriptions-item label="销售订单号"><CopyableNo :value="detail.order_no" /></el-descriptions-item>
            ...
            <el-descriptions-item label="发货职员">{{ emptyText(detail.employee_name) }}</el-descriptions-item>
            <el-descriptions-item label="发货总数">{{ formatQuantity(detail.delivery_total_quantity) }}</el-descriptions-item>
            <el-descriptions-item label="发货总额">¥{{ formatMoney(detail.total_amount) }}</el-descriptions-item>
            <el-descriptions-item label="税金总额">¥{{ formatMoney(detail.total_tax) }}</el-descriptions-item>
            ...
            <el-descriptions-item label="收货地址" :span="3">{{ emptyText(detail.detail_address) }}</el-descriptions-item>
            <el-descriptions-item label="发货备注" :span="3">{{ emptyText(detail.delivery_remark) }}</el-descriptions-item>
          </el-descriptions>

          <div class="detail-section-title">发货产品</div>
          <el-table :data="detail.items || []" border stripe>
            ...
            <el-table-column label="退货数量" width="110" align="right">
              <template #default="{ row }">{{ formatQuantity(row.returned_quantity) }}</template>
            </el-table-column>
          </el-table>
        </template>
      </div>
    ...
    <el-dialog v-model="warehouseDialogVisible" title="全部仓库发货数据" width="1100px">
      <el-table border :data="warehouseSummaries" stripe max-height="520">
        ...
        <el-table-column label="产品总数" width="110" align="right">
          <template #default="{ row }">{{ formatQuantity(row.product_total) }}</template>
        </el-table-column>
        <el-table-column label="发货总数" width="110" align="right">
          <template #default="{ row }">{{ formatQuantity(row.delivery_total) }}</template>
        </el-table-column>
        <el-table-column label="总额" width="130" align="right">
          <template #default="{ row }">¥{{ formatMoney(row.total_amount) }}</template>
        </el-table-column>
        <el-table-column label="总税金" width="130" align="right">
...
import { getSaleDeliveryReport, getSaleDeliveryWarehouseSummary } from '@/api/report'
import { getSaleDelivery } from '@/api/sale'
...
async function fetchData() {
  loading.value = true
  try {
    const res: any = await getSaleDeliveryReport(reportParams())
...
async function fetchWarehouseSummaries() {
  const res: any = await getSaleDeliveryWarehouseSummary(reportParams(false))
...
async function handleDetail(row: any) {
  ...
  try {
    const res: any = await getSaleDelivery(row.delivery_id)
    detail.value = {
      ...row,
      ...(res.data || {}),
