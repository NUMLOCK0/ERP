<template>
  <div class="report-page">
    <el-card>
      <SearchForm :model="searchForm" @search="handleSearch" @reset="handleReset">
        <el-form-item label="关键词">
          <el-input v-model="searchForm.keyword" placeholder="销售单号/产品名称/编码" clearable />
        </el-form-item>
        ...
        <div class="warehouse-cards">
          <div v-for="warehouse in visibleWarehouses" :key="warehouse.id" class="warehouse-card">
            ...
            <div class="warehouse-stat-grid">
              ...
              <span>销售总数 {{ formatQuantity(warehouse.sale_total) }}</span>
              <span>销售总额 ¥{{ formatMoney(warehouse.sale_amount) }}</span>
    ...
    <el-drawer v-model="detailVisible" title="销售订单详情" size="76%">
      <div v-loading="detailLoading" class="detail-content">
        <template v-if="detail">
          <el-descriptions :column="3" border>
            ...
            <el-descriptions-item label="销售职员">{{ emptyText(detail.employee_name) }}</el-descriptions-item>
            ...
            <el-descriptions-item label="收货地址">{{ emptyText(detail.detail_address) }}</el-descriptions-item>
            <el-descriptions-item label="管理员备注" :span="3">{{ emptyText(detail.admin_remark) }}</el-descriptions-item>
            <el-descriptions-item label="销售备注" :span="3">{{ emptyText(detail.sale_remark) }}</el-descriptions-item>
          </el-descriptions>

          <div class="detail-section-title">销售产品</div>
    ...
    <el-dialog v-model="warehouseDialogVisible" title="全部仓库销售订单数据" width="1050px">
      <el-table border :data="warehouseSummaries" stripe max-height="520">
        ...
        <el-table-column label="销售总数" width="120" align="right">
          <template #default="{ row }">{{ formatQuantity(row.sale_total) }}</template>
        </el-table-column>
        <el-table-column label="销售总额" width="140" align="right">
...
async function fetchData() {
  loading.value = true
  try {
    const res: any = await getSaleOrderReport(reportParams())
...
async function fetchWarehouseSummaries() {
  const res: any = await getSaleOrderWarehouseSummary(reportParams(false))
...
async function handleDetail(row: any) {
  ...
  try {
    const res: any = await getSaleOrder(row.order_id)
    detail.value = {
      ...row,
      ...(res.data || {}),
